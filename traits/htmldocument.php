 ...
 *
 * This trait contains only HTML methods, so it will not
...
trait HTMLDocument
{
	 ...
	 */
	final public function loadHTML($source, $options = 0)
	{
		return $this->ownerDocument->loadHTML($source, $options);
	}
	 ...
	 * @return bool             Returns TRUE on success or FALSE on failure.
	 */
	final public function loadHTMLFile($filename, $options = 0)
	{
		return $this->ownerDocument->loadHTMLFile($filename, $options);
	}
	 ...
	 */
	final public function saveHTML(\DOMNode $node = null)
	{
		return $this->ownerDocument->saveHTML($node);
	}
	 ...
	 */
	final public function saveHTMLFile($filename)
	{
		return $this->ownerDocument->saveHTMLFile($filename);
	}
}
