...
/**
 * Trait to provide \DOMDocument methods to any \DOMNode class,
 * by aliasing the method to `$node->ownerDocument` methods.
 *
 * This trait contains only XML methods, so it will not
...
trait XMLDocument
{
	 ...
	 */
	final public function load($filename, $options = 0)
	{
		return $this->ownerDocument->load($filename, $options);
	}
	 ...
	 * @return bool            Returns TRUE on success or FALSE on failure.
	 */
	final public function loadXML($source, $options = 0)
	{
		return $this->ownerDocument->loadXML($source, $options);
	}
	 ...
	 */
	final public function save($filename, $options = null)
	{
		return $this->ownerDocument-save($filename, $options);
	}
	 ...
	 */
	final public function saveXML(\DOMNode $node = null, $options = null)
	{
		return $this->ownerDocument->saveXML($node, $options);
	}
}
